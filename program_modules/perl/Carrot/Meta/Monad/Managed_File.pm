...
# //returns
{
	$_[THIS][ATR_NEEDS_UPDATE] = IS_FALSE;
	$_[THIS][ATR_PKG_MTIME] = IS_UNDEFINED;
	return;
}
...
# //returns
{
	...
	return unless (defined($this->[ATR_PKG_MTIME]));
	if ($file_name->exists)
	{
		my $file_mtime = $file_name->status->modification_timestamp;

		if ($this->[ATR_PKG_MTIME]->is_newer($file_mtime))
		{
			$this->[ATR_NEEDS_UPDATE] = IS_FALSE;
		} else {
